#!/usr/bin/env python3
"""
Merged Customer & Demographics Analysis Map
Combines census tract demographics with customer cluster analysis
"""
...
print("🗺️  Building Merged Customer & Demographics Analysis Map...")
print("=" * 70)
...
print(f"   Census tracts loaded: {len(df_census):,}")
...
print(f"   Customers loaded: {len(df_customers):,} total, {len(df_active):,} active")

# Create map
m = folium.Map(location=[40.0, -75.5], zoom_start=9, tiles='cartodbpositron', control_scale=True)

# ========================================
# LAYER 1 (BOTTOM): CENSUS TRACT DEMOGRAPHICS
# ========================================
print("\n🎨 Creating census tract demographic layers (BASELINE)...")
...
for demo, (layer_name, demo_type) in demographics_config.items():
    ...
    for idx, row in df_census.iterrows():
        if pd.notna(row['geometry']):
            ...
            # Apply exclusions for density and home value
...
# ========================================
# LAYER 2 (MIDDLE): CUSTOMER CLUSTERS
# ========================================
print("\n🎯 Creating customer cluster layers (ON TOP OF DEMOGRAPHICS)...")
...
print(f"   ✅ All clusters: {len(df_customers):,} points")

# ========================================
# LAYER 3 (TOP): GMB LOCATIONS - Always Shine Through
# ========================================
print("\n🏢 Creating GMB location markers (ALWAYS ON TOP)...")
...
for loc in current_locations:
    ...
    # Yellow marker with DivIcon (renders in markerPane - z-index 600)
    icon_html = f"""
...
<div style="position: fixed; top: 10px; left: 50px; width: 380px; height: auto;
     ...
     <h4 style="margin:0; color:#333; font-size:16px;">Customer & Demographics Analysis</h4>
</div>
'''
...
print("\n✅ Merged Customer & Demographics Analysis Map saved!")
print("=" * 70)
print("\n📋 LAYER HIERARCHY (bottom to top):")
print("   1. Census tract demographics (baseline)")
print("   2. Customer clusters (middle)")
print("   3. GMB locations (always on top)")
print("\n🎯 Result: GMB markers always shine through everything!")
