#!/usr/bin/env python3
"""
Add city names and bold markers - NO county boundary
"""

import folium
import pandas as pd

print("🗺️  Adding city names and bold markers (no county boundary)...")
print("=" * 70)
...
# Create demographic layers WITH CITY NAMES
demographics = {
...
for demo, (layer_name, color_col) in demographics.items():
    ...
    folium.GeoJson(
        ...
        tooltip=folium.GeoJsonTooltip(
            fields=['city', 'zip_code', 'value'],
            aliases=['City:', 'ZIP:', f'{demo.replace("_", " ").title()}:'],
            localize=True
        )
    ).add_to(layer)
...
print("   ✅ Layers created with city names")

# BOLD MARKERS with contrasting colors
current_locations = [
...
# Bold current locations - LIME GREEN
...
<div style="position: fixed; 
     ...
     <p style="font-size:10px; margin:0 0 8px 0; color:#666;"><b>Hover to see city name + data</b></p>
...
output_file = '/workspace/interactive-map-working.html'
m.save(output_file)

print(f"\n✅ Working map saved: {output_file}")
print(f"\n✅ Added:")
print(f"   - City names in tooltips")
print(f"   - Bold lime green markers (current)")
print(f"   - Bold cyan markers (prospects)")
print(f"   - NO county boundary (kept it simple)")
print("\n" + "=" * 70)
